...
- Creates `.venv/` and installs all Python dependencies (Flask, pytest, requests)
 ...
 > **For the demo, the command above is all you need. The options below are for reference only.**
...
1. Ensure `certs/wajntraub-demo-ca.key` and `certs/wajntraub-demo-ca.crt` are present in the repo on the provisioning machine (generated by the initial `setup.sh` run — do not delete them).
2. Run on the provisioning machine to generate a fresh key and cert:
   ```bash
   sudo ./scripts/setup.sh --new-device-cert
   ```
3. Copy the freshly generated `certs/device.key` and `certs/device.crt` to the new device.
4. Repeat from step 2 for each additional device — each run overwrites the previous pair in `certs/`.
