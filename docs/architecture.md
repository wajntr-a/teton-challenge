...
| `hostapd` | System daemon | Broadcasts the SoftAP SSID. Config in `device/hostapd.conf`. | AD-1 |
| `dnsmasq` | System daemon | DHCP on SoftAP subnet + DNS resolution for `setup.teton-device.local → 192.168.4.1`. Config in `device/dnsmasq.conf`. | AD-3 |
...
- **Network interfaces:** E2E tests use virtual radios created by `mac80211_hwsim`. Physical interfaces and existing NetworkManager connections are not touched.
...
| `teton-ca.key` (demo CA signing key) | Local filesystem only — generated by `setup.sh` | Never committed to git (gitignored); not needed after device cert is signed |
| `device.crt`, `teton-ca.crt` | `certs/` | Public certs — not secrets; `teton-ca.crt` is the file the evaluator installs |
...
- `certs/` gitignore exclusions — which generated files to exclude vs include
