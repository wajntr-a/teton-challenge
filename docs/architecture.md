...
| AD-4 | **Python 3 + Flask** | The provisioning server handles one GET and one POST then shuts down; no async required; immediately readable by any evaluator; stdlib covers everything needed (ssl, subprocess, threading) with no unusual dependencies | Rust (less mature ecosystem for this stack), FastAPI (unnecessary for 2 routes), Node.js (no benefit over Flask here) |
| AD-5 | **Software RSA key for TLS (demo); tpm2-pkcs11 for production** | Python's `ssl.SSLContext.load_cert_chain()` calls `SSL_CTX_use_PrivateKey_file()` which uses `fopen()` — it cannot resolve TPM store URIs. The tpm2-openssl provider hooks into OpenSSL's `OSSL_STORE` API, which Python's ssl module never invokes. Demo uses a software key; production path is tpm2-pkcs11 (PKCS#11 over TPM2), which IS reachable from Python ssl via `pkcs11:` URI — one line change, private key stays hardware-bound | tpm2-openssl provider directly (blocked by Python ssl internals), tpm2-tss-engine (deprecated, broken on Bookworm) |
